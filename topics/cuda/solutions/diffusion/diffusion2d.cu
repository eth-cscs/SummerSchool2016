...
void diffusion_shared(double *x0, double *x1, int nx, int ny, double dt) {
    ...
    auto ib = threadIdx.x + 1;
    auto jb = threadIdx.y + 1;

    auto nxb = blockDim.x+2;
    auto nyb = blockDim.y+2;
    ...
    if(i<nx-1 && j<ny-1) {
        ...
        if(ib==1)     buf[bpos-1] = x0[pos-1];
        if(ib==nxb-2) buf[bpos+1] = x0[pos+1];
        if(jb==1)     buf[bpos-nxb] = x0[pos-nx];
        if(jb==nyb-2) buf[bpos+nxb] = x0[pos+nx];

        __syncthreads();

        x1[pos] = buf[bpos] + dt * (-4.*buf[bpos]
                   + buf[bpos-nxb] + buf[bpos+nxb]
                   + buf[bpos-1]   + buf[bpos+1]);
    }
}
...
int main(int argc, char** argv) {
    ...
    for(auto step=0; step<nsteps; ++step) {
        ...
        #ifdef USE_SHARED
        auto shared_size =
            sizeof(double) * (block_dims.x+2) * (block_dims.y+2);
        diffusion_shared<<<grid_dims, block_dims, shared_size>>>(x0, x1, nx, ny, dt);
        #else
