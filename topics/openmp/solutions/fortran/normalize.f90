...
implicit none
    ...
    print *, 'serial     : ', time_serial, ' s, parallel   : ', time_parallel, ' s'
...
subroutine normalize_vector_omp(v, n)
    ...
    real(kind=8) :: norm_local = 0.0d0

    !$omp parallel firstprivate(norm_local)
        !$omp do
        do i=1,n
            norm_local = norm_local + v(i)*v(i);
        end do
        !$omp end do

        !$omp critical
        norm = norm + norm_local
        !$omp end critical
