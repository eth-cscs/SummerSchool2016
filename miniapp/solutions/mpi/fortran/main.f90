...
program diffusion_serial
    ...
    if (domain%comm_cart .ne. MPI_PROC_NULL) then
        call MPI_Comm_free(domain%comm_cart,ierr)
    end if
...
subroutine initialize_mpi(options, domain)
    ...
    periods(1) = .false.
    periods(2) = .false.
    call mpi_cart_create(MPI_COMM_WORLD, 2, dims, periods, 0, domain%comm_cart, ierr)

    ! retrieve coordinates of the rank in the topology
    call mpi_cart_coords(domain%comm_cart, mpi_rank, 2, coords, ierr)
    domy = coords(1)+1
    domx = coords(2)+1

    ! set neighbours for all directions
    call mpi_cart_shift(domain%comm_cart, 0, 1, domain%neighbour_south, domain%neighbour_north, ierr)
    call mpi_cart_shift(domain%comm_cart, 1, 1, domain%neighbour_west, domain%neighbour_east, ierr)
